...
# Ссылка на БД (рекомендую сменить пароль после тестов)
...
async def init_db():
    ...
    # Таблица для хранения связей: ID подключения -> ID пользователя (кому отправлять логи)
    await conn.execute('''
    ...
    # Таблица сообщений, привязанная к connection_id
    await conn.execute('''
...
# 3. Отслеживаем удаленные сообщения (Исправленный декоратор)
