...
class TrackedMessage(Base):
    ...
    message_type: Mapped[str] = mapped_column(String(50))  # incoming, outgoing, edit, reply
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "/start - Главное меню\n"
        "/help - Эта справка\n"
        "/chats - Список активных бизнес-чатов\n"
        "/status - Статистика бота\n\n"
        "Просто подключите бота в Telegram Business, и он начнёт работу!",
        parse_mode=ParseMode.MARKDOWN,
    )
        ...
        if business_connection.is_enabled:
            ...
            existing_chat = await session.execute(
                select(BusinessChat).where(
                    BusinessChat.business_connection_id == business_connection.id
                )
            )
            ...
            if not chat:
                ...
                logger.info(f"✅ Новый бизнес-чат подключён: {chat.id}")

                # Создаём настройки уведомлений для пользователя
                ...
                try:
                    await bot.send_message(
                        ...
                        f"ID подключения: `{business_connection.id[:16]}...`\n"
                        f"Чат #{chat.id} теперь отслеживается.\n\n"
                        f"Используйте /chats для просмотра настроек.",
                        parse_mode=ParseMode.MARKDOWN,
                    )
                except Exception as e:
                    logger.error(f"Не удалось отправить уведомление: {e}")
            else:
                # Переподключение существующего чата
                chat.is_active = True
...
# Обработка редактирования бизнес-сообщений
@router.edited_business_message()
async def on_edited_business_message(message: Message):
    """Обработчик редактирования бизнес-сообщений"""
    if not message.business_connection_id:
        return
    ...
    async with async_session() as session:
        ...
        if tracked_msg:
            # Обновляем историю редактирования
...
async def get_business_chat(session: AsyncSession, business_connection_id: str) -> Optional[BusinessChat]:
    """Получение бизнес-чата по connection ID"""
...
async def notify_user(session: AsyncSession, chat: BusinessChat, message: TrackedMessage, event_type: str):
    ...
    try:
        ...
        logger.error(f"Не удалось отправить уведомление пользователю {chat.user_id}: {e}")


async def list_user_chats(message: Message, user_id: int, edit: bool = False):
    """Показать список бизнес-чатов пользователя"""
    async with async_session() as session:
        ...
        if not chats:
            ...
            for chat in chats:
                ...
                builder.button(
                    text=f"⚙️ Чат #{chat.id}",
                    callback_data=f"settings:{chat.id}"
                )
...
async def show_bot_status(message: Message, edit: bool = False):
    ...
    async with async_session() as session:
        total_chats = await session.execute(
            select(func.count(BusinessChat.id))
        )
        ...
        total_messages = await session.execute(
            select(func.count(TrackedMessage.id))
        )
...
async def show_chat_history(message: Message, chat_id: int, page: int = 0, user_id: int = None, edit: bool = False):
    """Показать историю сообщений чата"""
    per_page = 5
    ...
    async with async_session() as session:
        ...
        text += f"Страница {page + 1}/{(total_count + per_page - 1) // per_page}\n\n"
        ...
        if edit:
            await message.edit_text(
                text,
                reply_markup=get_history_keyboard(chat_id, page),
                parse_mode=ParseMode.MARKDOWN,
            )
        else:
            await message.answer(
