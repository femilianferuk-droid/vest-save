...
# Удаление бизнес-сообщений — через обработчик в Dispatcher
async def on_business_messages_deleted(event, bot_instance, **kwargs):
    """Обработчик удаления бизнес-сообщений"""
    if not hasattr(event, 'business_connection_id') or not event.business_connection_id:
        return
...
# Регистрируем обработчик удаления бизнес-сообщений через Dispatcher
from aiogram.types import BusinessMessagesDeleted
dp.business_messages_deleted.register(on_business_messages_deleted, BusinessMessagesDeleted)
